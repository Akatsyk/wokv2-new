...
void c_visuals::on_paint() {
	for (auto i = 0; i <= 64; i++) {
		...
		if (player->get_health() <= 0 || player->get_origin() == vec3_t(0, 0, 0))
			continue;
...
void c_visuals::draw_box(c_cs_player* player, vec3_t pos, vec3_t top, col_t color) {
	float alpha = 255.f;
...
void c_visuals::draw_name(c_cs_player* player, int index, vec3_t pos, vec3_t top, col_t color) {
	...
	render::text_string(pos.x - size.x / 2, top.y - 15, color, e_font_flags::FONT_OUTLINE, fonts::m_tahoma14, player_info.m_name);
}

void c_visuals::draw_health(c_cs_player* player, vec3_t pos, vec3_t top) {
